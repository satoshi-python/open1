...
def cluster_move_emsm(cluster, name2, dime1):
    SCORE = []
    SCORE_cv = []
    #dime1 = np.concatenate(dime)
    #a2 = [int(i) for i in range(15,26)]
    clusters = len(cluster.cluster_centers_)
    LAG = [50, 100, 200, 500, 1000]
    for ll in LAG:
        try:
            dtrajs_concatenated = cluster.labels_
            clusters = len(cluster.cluster_centers_)
            msm = pyemma.msm.estimate_markov_model(cluster.labels_, lag=int(ll), dt_traj='1 ps')
            MSM_vac(msm, "estimate_markov_model_cluster_number"+str(clusters)+"_LAG" + str(ll))
            SCORE.append([clusters, ll, msm.score(cluster.labels_)])
            SCORE_cv.append([clusters, ll, msm.score_cv(cluster.labels_)])
            ...
            for i in cluster.cluster_centers_:
                coarse_state_centers.append(list(i))
            coarse_state_centers = np.array(coarse_state_centers)
            plot_LABEL(clusters, flux, highest_membership, coarse_state_centers, dime1[0], dtrajs_concatenated, name2, ll, "estimate_markov_model", msm)
...
def cluster_move_bmsm(cluster, name2, dime1):
    SCORE = []
    SCORE_cv = []
    LAG = [50, 100, 200, 500, 1000]
    for ll in LAG:
        try:
            ...
            dtrajs_concatenated = np.concatenate(cluster.labels_)
            msm = pyemma.msm.bayesian_markov_model(cluster.labels_, lag=int(ll), dt_traj='1 ps')
    ...
    try:
        for nstates in a2:
            ...
            for ll in LAG:
                ...
                plot_LABEL(nstates, flux, highest_membership, coarse_state_centers, dime1, dtrajs_concatenated[0], name2, ll, "estimate_hidden_markov_model", msm)
